...
async function testFieldExtraction() {
  ...
  const extractionTests = [
    ...
    {
      name: 'Extract budget with "mensual"',
      message: 'mi presupuesto es $800 mensual',
      expectedFields: ['budget'],
      expectedValues: { budget: 800 }
    },
    {
      name: 'Extract email',
      message: 'mi correo es carlos@restaurante.com',
      expectedFields: ['email'],
      expectedValues: { email: 'carlos@restaurante.com' }
    },
    {
    ...
    try {
      const result = await salesAgent.invoke(state, {
        configurable: {
          ...
          thread_id: `test-thread-${Date.now()}`  // Required for MemorySaver
        },
...
async function testRecursionProtection() {
  ...
  try {
    const result = await salesAgent.invoke(duplicateState, {
      configurable: {
        ghlService: mockGhl,
        calendarId: 'test',
        contactId: duplicateState.contactId
      },
      runId: crypto.randomUUID(),
        thread_id: `test-thread-${Date.now()}`
    });
...
async function testStateManagement() {
  ...
  const result1 = await salesAgent.invoke(state, {
    configurable: {
      ghlService: mockGhl,
      calendarId: 'test',
      contactId: state.contactId
  ...
  const result2 = await salesAgent.invoke(result1, {
    configurable: {
      ghlService: mockGhl,
      calendarId: 'test',
      contactId: state.contactId
  ...
  const [result1Conv, result2Conv] = await Promise.all([
    salesAgent.invoke(conv1, {
      configurable: { ghlService: mockGhl1, calendarId: 'test', contactId: 'contact-1' },
      runId: crypto.randomUUID(),
        thread_id: `test-thread-${Date.now()}`
    }),
    salesAgent.invoke(conv2, {
      configurable: { ghlService: mockGhl2, calendarId: 'test', contactId: 'contact-2' },
      runId: crypto.randomUUID(),
        thread_id: `test-thread-${Date.now()}`
    })
  ]);
  ...
  const toolTests = [
    {
      ...
      test: async () => {
        ...
        await salesAgent.invoke(state, {
          configurable: { ghlService: mockGhl, calendarId: 'test', contactId: state.contactId },
          runId: crypto.randomUUID(),
        thread_id: `test-thread-${Date.now()}`
        });
    ...
    },
    {
      ...
      test: async () => {
        ...
        const result = await salesAgent.invoke(state, {
          configurable: { 
            ghlService: createMockGhlService(), 
            calendarId: 'test', 
            contactId: state.contactId 
          },
          runId: crypto.randomUUID(),
...
async function testEdgeCases() {
  ...
  const edgeCases = [
    ...
    },
    {
      ...
      messages: ['a'.repeat(1000)],
      expectedBehavior: 'Should process without error'
    },
    {
    ...
    },
    {
      ...
      messages: ['Hola 😊 soy Maria 🌟'],
      expectedBehavior: 'Should extract name without emojis'
    }
  ];
